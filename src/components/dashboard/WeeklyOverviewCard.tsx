    ...
    <GlassCard className="space-y-5 p-5">
      ...
      {totalHours === 0 ? (
        <p className="py-8 text-center text-sm text-ink-secondary">
          No focus sessions logged yet. Start the Focus Timer to see your progress here.
        </p>
      ) : (
        <div className="flex h-36 items-end justify-center gap-4">
          {weekData.map((entry, index) => (
            <div key={`${entry.day}-${index}`} className="flex flex-col items-center gap-2">
              <div className="flex h-28 w-2.5 items-end overflow-hidden rounded-full bg-tint/5 transition-transform duration-200 hover:scale-105">
                <div
                  className={cn(
                    ...
                    entry.day === peakDay.day && entry.hours === peakDay.hours && 'from-secondary to-highlight',
                  )}
                  style={{ height: `${Math.max((entry.hours / maxHours) * 100, entry.hours > 0 ? 10 : 2)}%` }}
                />
              </div>
      ...
      <div>
        <p className="font-mono text-2xl font-semibold text-ink">{totalHours.toFixed(1)}h</p>
